...
def CalcRelRates(Ibeam,gam,ex,ey,sgms,sgmp,
    lattice_df,
    showLcMinMaxMean = False):
    emx = 1e-4*ex
    emy = 1e-4*ey
    ...
    dz_arr = lattice_df['dS']
    bet_x = lattice_df['Beta_cm_X']
    D_x = lattice_df['Dispersion_cm_X']
    Phi_x = lattice_df['Phi_X']
    bet_y = lattice_df['Beta_cm_Y']
    dz_arr, bet_x, D_x, Phi_x, bet_y = (dz_arr.to_list(), bet_x.to_list(), D_x.to_list(), Phi_x.to_list(), bet_y.to_list())
